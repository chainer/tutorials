...
# ここで、初めの図にあった赤い点の集まりに見られる $x$ と $t$ の関係を、うまく式 $\eqref{line}$ の直線によって近似することを考えてみましょう。
#
...
#
# 式 $\eqref{line}$ で表される直線は、$w$ と $b$ が決まると、どのような直線になるかが決定されます。
...
#
# また、パラメータ $w$ と $b$ によって特徴づけられた直線の式  $\eqref{line}$ のような、パラメータを使って何らかの計算を行うことで、与えられたデータの特徴や関係性を表すものを、以降簡単に**モデル (model)** と呼ぶことにします（[注釈1](#note1)）。
#
...
# まず、式 $\eqref{line}$ にある $x$ を与えると、$w$ と $b$ を使って $wx + b$ という計算が行われます。
# その結果を $y$ と表すことにします。
# ここで、$x$ を**入力変数 (input variable)**、$y$ を**出力変数 (output variable)** と呼びます。
# 今、上図の赤い点の集まりから 1 点を選び出し、その $x$ 座標の値を式 $\eqref{line}$ に与えたとします。
# このとき、式 $\eqref{line}$ に与えられた具体的な $x$ の値を、**入力値 (input value)** と呼びます。
...
#
# そして、この 200 個の $x$ の値全てに対して、式 $\eqref{line}$ による予測値 $y$ を計算した結果、$y_1, y_2, \dots, y_{200}$ が得られたとします。
#
