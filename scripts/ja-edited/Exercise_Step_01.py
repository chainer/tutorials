...
# # Step 1 の演習問題
#
# 本章では Step 1 で扱ったテーマについて演習問題を記します。
# 本章の演習問題には Step 1 で扱った知識だけで解けるものもあれば、外部の資料を調べる必要があるものもあります。
#
# -
...
#
# Python コード上でベクトルや行列に関する演算を行うことを考えてみましょう。ベクトルは `float` のリスト、行列は `float` のリストのリストによって表すことができます。例えば、
#
# $$
# {\bf x}=\begin{bmatrix}
# 3 \\
...
# ### 問5.2 (ベクトルの微分)
# ${\bf x}$ を次元数 $n$ のベクトル、${\bf A}$ を $n \times n$ の正方行列とします。
#
# $$
# {\bf x}=\begin{bmatrix}
# x_{1} \\
...
# ### 問5.5 (多変数関数の合成関数の微分)
#
# 3次元ベクトルに対するベクトル値関数 ${\bf f}$ を以下によって定義します。
# $$
# {\bf f}(\begin{bmatrix}
# x_{1} \\
# x_{2} \\
...
# ### 問5.6 (多変数関数の合成関数の微分)
# ${\bf A}$, ${\bf B}$ を $n \times n$ 行列とします。$n$ 次元ベクトル ${\bf x}$ に対して ${\bf f}({\bf x})={\bf A}{\bf x}$, ${\bf g}({\bf x})={\bf B}{\bf x}$ とします。
#
# 合成関数の微分則を用いて $\frac{\partial}{\partial {\bf x}}{\bf g}({\bf f}({\bf x})) = {\bf B}{\bf A}$ を示して下さい。

# ### 問5.7 (多変数関数の合成関数の微分)
#
# (1) $f(y, z) = 2y-3z$ とします。このとき、1変数関数 $g(x)$ と $h(x)$ に対して
#
# $$
...
# ### 問6.5 (尤度と最尤推定)
#
# 偏りのあるサイコロの出る目の確率を推定する問題を考えます。サイコロを振ったときに出る数を表す確率変数を $X$ とします。確率モデルのパラメータを $\theta=(\theta_1, \theta_2, \theta_3, \theta_4, \theta_5, \theta_6)$ として、確率 $p(X=x)$ を
#
# $$
# f(x; \theta) = \theta_x
# $$
...
# ### 問6.6 (尤度と最尤推定)
#
# ポアソン分布は正の実数 $\lambda>0$ をパラメータとして持つ分布です。$0$ 以上の整数値を取る確率変数 $X$ が以下の確率値に従うとき、$X$ はポアソン分布に従うと言います。
#
...
#
# 6章の例6.7.1 にあるコイントスの試行で、事前確率 $p(\theta)$ を以下のように変更した場合にどうなるかを考えてみます。
#
# $$
# p(\theta)=\begin{cases}
